...
| **PFRDA/EPFO real-PYQ drill mode** | P0 | New `/nyaya` page + `backend/routes/nyaya_pyq_drill.py` + `backend/nyaya_core_client.py` — real, previously-asked PFRDA/EPFO questions (from sibling nyaya-core project's `core.db`, 470 + 660 real MCQs) served, graded, and coverage-tracked live over HTTP; zero LLM calls, zero new tables in this repo. Existing UPSC Prelims `quiz.py`/Chroma path untouched. AI-generated Mode 2 (matching UPSC Prelims' dimension-based generation) deliberately deferred — see HANDOFF.md. | Built 2026-09-17, branch `feature/pfrda-epfo-nyaya-core-integration`, not yet merged |
