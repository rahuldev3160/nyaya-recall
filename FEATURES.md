...
| **Full Mock exam-simulation mode (Feature #13)** | P0 | Fixed 100-question/120-minute mode inside exam-sim, real-PYQ-first (proportional to new `topic_weights` seed matching the real 2026 UPSC paper's subject split), reserves used PYQs from regular practice (`mock_reserved_questions`), AI-fill only for gaps with `source_type` tagged so results honestly report % real PYQ vs AI. Also fixes ISSUE-028 (exam-sim's AI generation was silently broken at the SDK level — plausibly why it's barely been used). Calibration fix in `prompts/exam_simulation.txt`: scenario/dilemma question type + qualitative UPSC-trickiness instructions replacing the flat difficulty label. | Built 2026-08-30, PR #56 (open, awaiting review/merge) |
| **PFRDA/EPFO real-PYQ drill mode (Mode 1)** | P0 | New `/nyaya` page + `backend/routes/nyaya_pyq_drill.py` + `backend/nyaya_core_client.py` — real, previously-asked PFRDA/EPFO questions (from sibling nyaya-core project's `core.db`, 470 + 660 real MCQs) served, graded, and coverage-tracked live over HTTP; zero LLM calls, zero new tables in this repo. Existing UPSC Prelims `quiz.py`/Chroma path untouched. | Built 2026-09-17, PR #57 merged to main |
| **PFRDA/EPFO AI-generated quiz mode (Mode 2)** | P1 | `POST /quiz/generate` branches on `config.exam_id` (top of the single endpoint, before any existing subject_id/subtopic_id logic runs) into `_generate_quiz_nyaya_core()` — grounds generation in nyaya-core's `/topic/{id}/brief` (real PYQs + any indexed explanation chunks), raises HTTP 422 if neither exists for a topic (never falls back to the UPSC stub text), 502 if nyaya-core is unreachable. Reuses `_get_quiz_intelligence`/`_build_recent_questions_block`/quiz_sessions INSERT as-is (both are generic by subject_id/subtopic_id string, no UPSC-specific assumptions). New flat-JSON dimension loader (`_get_topic_dimensions_nyaya_core`) for `data/dimensions/{pfrda_gradea,upsc_epfo_apfc_eo_ao}.json` (different shape from syllabus.json's nested tree). New prompt `prompts/pfrda_epfo_quiz.txt`. UI: `/nyaya` page's topic list now offers "Real PYQs" and "AI Quiz" per topic (new `ai_quiz`/`ai_done` views, client-side answer check, same convention as `diagnostic/page.tsx`). Existing UPSC Prelims `/quiz/generate` callers (no `exam_id` in payload) verified byte-identical — old code path only shifted down 8 lines, zero logic changes. **Real finding:** nyaya-core has zero indexed chunks for either exam (`scripts/inventory.py`) — `/search`/`/topic/{id}/brief` return `insufficient_grounding=True` for every PFRDA/EPFO topic today, so grounding is currently 100% real-PYQ-based, not explanation-chunk-based; the 422 hard-error path only fires when a topic also has zero real PYQs (confirmed live: `pfrda_english_language`, a paper-level rollup with no directly-tagged PYQs). | Built 2026-09-18, branch `feature/pfrda-epfo-mode2-ai-quiz`, not yet merged |
...
| ~~13~~ | ~~Mock test mode~~ | ~~P3~~ | ~~SHIPPED 2026-08-30 as "Full Mock exam-simulation mode" — see ✅ Shipped table above, PR #56~~ | ~~PR #56~~ |
...
| 22 | **PFRDA/EPFO AI-generated quiz (Mode 2)** | **P1** | Give PFRDA/EPFO the same adaptive/dimension-based AI generation `quiz.py` gives UPSC Prelims. Needs real `exam_id` parameterization threaded through `fetch_chunks_merged`/`_build_merged_content_chunks_str`/dimension lookups — genuinely large, `quiz.py` has no single branchable choke point today (confirmed by reading the full file, not assumed). Real-PYQ-grounded dimensions are already built (`scripts/generate_dimensions_from_pyqs.py`, nyaya-core-backed) — this item is just the generation-engine wiring. Real-PYQ drill (Mode 1, zero AI) already shipped — see ✅ Shipped table. | `HANDOFF.md 2026-09-17` |
