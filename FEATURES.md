...
| **PFRDA/EPFO real-PYQ drill mode** | P0 | New `/nyaya` page + `backend/routes/nyaya_pyq_drill.py` + `backend/nyaya_core_client.py` — real, previously-asked PFRDA/EPFO questions (from sibling nyaya-core project's `core.db`, 470 + 660 real MCQs) served, graded, and coverage-tracked live over HTTP; zero LLM calls, zero new tables in this repo. Existing UPSC Prelims `quiz.py`/Chroma path untouched. | Merged 2026-09-17, PR #57 |
| **PFRDA/EPFO drill: 5 real bugs fixed + AI-generated bank badge** | P0 | Fixed on Rahul's first real use: broken/single-option MCQs now filtered (`_is_answerable_mcq`), dead-end/duplicate topics removed from the picker, weighted-random repeat-aware question selection (new nyaya-core `GET /attempts`), real "Exit quiz"/"← Previous" navigation (per-question answer state). Also fixed a pre-existing crash blocking every page (Supabase never configured). Wires in nyaya-core's new persisted AI-generated question bank with a clear "AI-Generated" vs "Real PYQ" badge — no backend change needed, the drill's validity filter already accepts any `pyq_bank` content generically. See ISSUE-030 through ISSUE-034. | Merged 2026-09-18, PR #60 |
