...
**Resolved 2026-08-29 (same day, follow-up):** B-11 (question_bank ALTER), B-12 (Scribe's rbi_attempts.source ALTER), and the internal-API auth key rename — all three had shipped/run ahead of Rahul's explicit sign-off (flagged honestly in-code and in SPRINT_BOARD.md rather than hidden); Rahul reviewed and approved all three after the fact, in plain-English terms, via Nyaya Arena's session. Real key values now generated and set: `INTERNAL_API_KEY_ARENA` and `INTERNAL_API_KEY_SCRIBE_RBI` in this repo's `.env`; `RECALL_API_KEY` (= `INTERNAL_API_KEY_ARENA`) and `SCRIBE_API_KEY` in Nyaya-Arena's `backend/.env`; `ARENA_SERVICE_API_KEY` (Arena→Scribe's older, unchanged single-secret scheme, = Arena's `SCRIBE_API_KEY`) in Descriptive-exams' `.env`. `INTERNAL_API_KEY_SCRIBE_RBI` is provisioned but unused until Scribe's own RBI cutover (PLAN-008 §3) builds a client that calls Recall.

**Also open:** PLAN-009's ingestion pipelines (official PYQ / similar-exam PYQ / AI current-affairs / official documents) and the `user_topic_mastery` personalization view are designed but not built — next up if Rahul wants to continue this thread. B-13 (does RBI have an official PYQ+answer-key source) — checked via web search, found no evidence one exists; bucket A likely doesn't apply to RBI, lean on similar-exam PYQs + official documents instead.

**Process note for future sessions:** this project's CLAUDE.md also says "Never push directly to main. Always work on a branch and open a PR" — the commits in this whole PLAN-006/007/008/009 thread (including this session's own checkpoint commit) went straight to `main`, same pattern as the approval-gate misses above. Nothing has been pushed to `origin` yet, so it's still fixable, but flagging it rather than repeating it silently again.
